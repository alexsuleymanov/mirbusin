                        ...
                        if($this->args[0] == 'search'){
                            ...
                            foreach ($cats as $sk => $cat_r) {
                                ...
                                foreach($words as $k => $v){
                                    $where .= " and (art like '%$v%' or name like '%$v%')";
                                }
                                $prodcounts[$cat_r->id] = $Prod->getnum(array(
                                    "where" => "visible = 1 and (num > 0 or num2 > 0 or num3 > 0)".$where,
                                    "relation" => array(
                                        "select" => "relation",
                                        "where" => "`type` = 'cat-prod' and obj = '" . $cat_r->id . "'"
                                    ),
                                                    ...
                                                    <li class="sc2"<?if($p++%3==0){?> style="clear: both;"<?}?>>
                                                        ...
                                                        <a href="/catalog/<?=$supurl?>cat-<?=$scat_r->id?>-<?=$scat_r->intname?>"<?=($this->cat==$scat_r->id)?' class="sub-active"':''?>
                                                           ...
                                                           data-id="<?= $scat_r->id ?>">
                                                            <?=$scat_r->name?>
                                                            <?
                                                            if (($this->args[1]=='new')||($this->args[1]=='pop')||($this->args[1]=='action')||($this->args[1]=='mix')||($this->args[1]=='onsale')||($this->args[0]=='search')) {
                                                                echo '('.$prodcount.')';
                                                            }
                                                            ?>
